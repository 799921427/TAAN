...
class BaseTrainer(object):
    def __init__(self, model_generator_I, model_discriminator, criterion_z, criterion_I, criterion_D, criterion_T, trainvallabel, a, b, c, u, k,t):
    ...
    def train(self, epoch, data_loader, optimizer_generator_I, optimizer_discriminator, print_freq=1):
        ...
        for i, inputs in enumerate(data_loader):
            ...
            # Calc the loss
            loss_t, loss_id, loss_discriminator, loss_T = self._forward(inputs, label, sub)

            L = self.a * loss_t + self.b * loss_id - self.c * loss_discriminator + self.t *loss_T
...
class rank_trainer(BaseTrainer):
    def _parse_data(self, inputs):
    ...
    def _forward(self, inputs, label, sub):
        outputs, outputs_pool = self.model_generator_I(inputs)

        loss_t , _ = self.criterion_I(outputs_pool, label, sub)

        loss_T = self.criterion_T(outputs_pool, sub, label)
        ...
        return  loss_t, loss_id, loss_discriminator, loss_T
