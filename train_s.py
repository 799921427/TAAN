...
from reid.trainer_wo_D import Trainer
from reid.evaluators import Evaluator
from reid.utils.data import transforms as T
from reid.utils.data.preprocessor import Preprocessor
from reid.utils.data.sampler import IDRandomIdentitySampler as RandomIdentitySampler
...
def get_data(name, split_id, data_dir, height, width, batch_size, num_instances,
    ...
    query_loader = DataLoader(
        ...
        batch_size=32, num_workers=workers,
        sampler=CamSampler(list(set(dataset.query)), [0,1,2,3]),
        shuffle=False, pin_memory=True)
    #
    gallery_loader = DataLoader(
        Preprocessor(list(set(dataset.gallery)),
                     root=dataset.images_dir, transform=test_transformer),
        batch_size=32, num_workers=workers,
        sampler=CamSampler(list(set(dataset.gallery)), [0,1,2,3], 4),
        shuffle=False, pin_memory=True)
...
def main(args):
    ...
    model = models.create(args.arch, num_classes=num_classes, num_features=args.features, attention_mode=args.att_mode)
    print(model)
    model = model.cuda()
    ...
    if args.arch == 'ide':
        ...
        ignored_params = list(map(id, model.classifier.parameters())) + list(map(id, model.attention_module.parameters()))
    ...
    if args.use_adam:
        ...
        optimizer_ft = torch.optim.Adam([
            ...
            {'params': model.attention_module.parameters(), 'lr': args.lr},
            ],
            weight_decay=5e-4)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mile_stone', type=list, default=[3000,4000])
