...
from reid.trainers_rank import Trainer
...
from utlis import RandomErasing, WarmupMultiStepLR,CrossEntropyLabelSmooth, Rank_loss
...
def main(args):
    ...
    criterion_z_s = CrossEntropyLabelSmooth(num_classes = num_classes, epsilon=0.3).cuda()
    ...
    trainer = Trainer(model_s, model_ir, model_t, criterion_z, criterion_I, criterion_att, trainvallabel, 1, 1, 0.0, 0.0, 1000)
    flag = 1
    best_top1 = -1
    best_top1_s = -1
    best_top1_ir = -1
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        }, False, epoch, args.logs_dir, fpath='s_checkpoint.pth.tar')

        save_checkpoint_ir({
            'model': model_ir.module.state_dict(),
            'epoch': epoch + 1,
            'best_top1': best_top1_ir,
        }, False, epoch, args.logs_dir, fpath='ir_checkpoint.pth.tar')
        # if epoch < 200:
        #     continue
        if not epoch % 1 ==0:
            continue
...
if __name__ == '__main__':
    ...
    parser.add_argument('--margin_2', type=float, default=1.5,
                        help="margin_2 of the triplet loss default: 0.5")
    ...
    parser.add_argument('--mile_stone', type=list, default=[100,550])
